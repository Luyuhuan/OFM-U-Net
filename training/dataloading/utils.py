...
def get_case_identifiers(folder: str) -> List[str]:
    ...
    if lzz_flag == 1:
        # 全部加载为训练集和验证集
        # datalistpath = os.path.join(folder, "datalist.csv")
        # case_identifiers = read_csv(datalistpath)
        ...
        traindatalistpath = os.path.join(folder, "traindatalist.csv")
        traincase_identifiers = read_csv(traindatalistpath)
        valdatalistpath = os.path.join(folder, "valdatalist.csv")
        # valdatalistpath = os.path.join(folder, "traindatalist.csv")
        valcase_identifiers = read_csv(valdatalistpath)
        case_identifiers = [traincase_identifiers, valcase_identifiers]
        return case_identifiers
