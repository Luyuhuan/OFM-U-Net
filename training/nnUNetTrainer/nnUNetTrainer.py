...
from nnunetv2.training.loss.compound_losses import DC_and_CE_loss, DC_and_BCE_loss, Regre_loss, Classification_loss
...
def save_images(data, save_folder, name):
    # 确保保存文件夹存在
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        # # test
        # self.num_iterations_per_epoch = 4
        # self.num_val_iterations_per_epoch = 4
        # self.num_iterations_per_epoch = 352
        # self.num_val_iterations_per_epoch = 4
        # batch 为 4
        self.num_iterations_per_epoch = 88
        self.num_val_iterations_per_epoch = 22
        # batch 为 2
        # self.num_iterations_per_epoch = 176
        # self.num_val_iterations_per_epoch = 44
        # self.num_iterations_per_epoch = 35
        # self.num_val_iterations_per_epoch = 9
        # self.num_iterations_per_epoch = 17
        # self.num_val_iterations_per_epoch = 5
        # batch 为 10
        # self.num_iterations_per_epoch = 36
        # self.num_val_iterations_per_epoch = 9
        ...
        self.class_loss = None  # -> self.initialize
    ...
    def initialize(self):
        if not self.was_initialized:
            ...
            self.loss = self._build_loss(mode = 0)
            self.class_loss = self._build_loss(mode = 1)
    ...
    def _build_loss(self, mode = None):
        global lzz_flag
        ...
        if lzz_flag == 1 and mode == 0:
            loss = Regre_loss(w1 = 1, w2 = 1, w3 = 1)
        if lzz_flag == 1 and mode == 1:
            loss = Classification_loss(w1 = 1, w2 = 1, w3 = 1)
    ...
    def configure_rotation_dummyDA_mirroring_and_inital_patch_size(self):
        ...
        initial_patch_size = get_patch_size(patch_size[-dim:],
                                            *rotation_for_DA.values(),
                                            (0.85, 1.25))
        if lzz_flag ==1:
            initial_patch_size = np.array(patch_size)
        if do_dummy_2d_data_aug:
    ...
    def do_split(self):
        ...
        if self.fold == "all":
            ...
            global lzz_flag
            if lzz_flag == 1:
                tr_keys = case_identifiers[0]
                val_keys = case_identifiers[1]
        else:
    ...
    def get_training_transforms(
        ...
        tr_transforms.append(SpatialTransform(
            ...
            random_crop=True,  # random cropping is part of our dataloaders
    ...
    def train_step(self, batch: dict) -> dict:
        global lzz_flag
        data = batch['data']
        # save_folder = "/root/lzz2/cardiac_cycle/mamba/try_umamba/U-Mamba/data/show/new"  # 保存图像的文件夹路径
        # save_images(data.detach().cpu().numpy(), save_folder, batch['keys'][0])
        target = batch['target']
        if lzz_flag == 1:
            #  只训练一层
        ...
        with autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
            ...
            classhead_flag = 0
            if classhead_flag == 1:
                output, output_class = self.network(data)
            else:
                output = self.network(data)
            if len(torch.nonzero(target[0] == 2.0)):
                print(" 还有label为2！ ")
            del data
            # 直接在求loss的时候处理为2的target
            # if lzz_flag == 1 and len(torch.nonzero(target[0] == 2.0)):
            #     now_2_indices = torch.nonzero(target[0] == 2.0)[0][1]
            #     target = [nowtarget[:,:now_2_indices,:] for nowtarget in target]
            #     output = [nowoutput[:,:now_2_indices,:] for nowoutput in output]
            # if len(torch.nonzero(target[0] == 2.0)):
            #     print(" 还有label为2！ ")
            l = self.loss(output, target)
            if classhead_flag == 1:
                l_class = self.class_loss(output_class, target)
                l = l + 0.3 * l_class
                # l = l + l_class
    ...
    def validation_step(self, batch: dict) -> dict:
        ...
        if lzz_flag == 1:
            ...
            # output = output[0]
            mse = mean_squared_error(target.squeeze(-1).detach().cpu(), output.squeeze(-1).detach().cpu())
            # r2 = r2_score(target.squeeze(-1).detach().cpu(), output.squeeze(-1).detach().cpu())
            r2 = r2_score(target.reshape(-1).detach().cpu(), output.reshape(-1).detach().cpu())
            # mse = sum([mean_squared_error(nowtarget.squeeze(-1).detach().cpu(), nowoutput.squeeze(-1).detach().cpu()) for nowtarget,nowoutput in zip(target,output)])
            # r2 = sum([r2_score(nowtarget.squeeze(0).squeeze(-1).detach().cpu(), nowoutput.squeeze(0).squeeze(-1).detach().cpu()) for nowtarget,nowoutput in zip(target,output)])
            # print("mse:  ", mse)
            # print("r2:  ", r2)
            self.mse.append(mse)
            self.r2.append(r2)
    ...
    def on_validation_epoch_end(self, val_outputs: List[dict]):
        ...
        if lzz_flag == 1:
            meanmse = mean(self.mse)
            meanr2 = mean(self.r2)
            self._all_r2.append(meanr2)
            self._now_r2 = meanr2
            self.print_to_log_file('meanmse:      ',meanmse)
            self.print_to_log_file('meanr2:       ',meanr2)
    ...
    def on_epoch_end(self):
        ...
        if lzz_flag == 1:
            if self._now_r2 == max(self._all_r2):
                # print("self._now_r2: ",self._now_r2, "   max(self._all_r2):  ", max(self._all_r2))
                self.print_to_log_file(f"Yayy! New best R2: {np.round(self._now_r2, decimals=4)}")
