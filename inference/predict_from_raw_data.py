...
videopath = r"/root/lzz2/cardiac_cycle/data/all_data_apex_direction/Apical"
jsonpath = r"/root/lzz2/cardiac_cycle/data/all_data_regressionlabel_apex_direction/Apical"
# videopath = r"/root/lzz2/cardiac_cycle/data/all_data_apex_direction/Parasternal"
# jsonpath = r"/root/lzz2/cardiac_cycle/data/all_data_regressionlabel_apex_direction/Parasternal"
# videopath = r"/root/lzz2/cardiac_cycle/data/all_data_apex_direction/Basal"
# jsonpath = r"/root/lzz2/cardiac_cycle/data/all_data_regressionlabel_apex_direction/Basal"
...
class nnUNetPredictor(object):
    ...
    def predict_from_files(self,
        ...
        if output_folder is not None:
            ...
            classflag = 0
            if classflag == 1:
                maybe_mkdir_p(output_folder.replace("val_pre","val_class_pre"))
        ...
        with multiprocessing.get_context("spawn").Pool(num_processes_segmentation_export) as export_pool:
            ...
            for preprocessed in data_iterator:
                ...
                if lzz_flage == 1:
                    ...
                    classpredata = []
                    savecsv = ofile + ".csv"
                    with open(savecsv, 'w', newline='') as csvfile:
                        writer = csv.writer(csvfile)
                        for nowframe,nowdata in zip(data[0],data[1]):
                            if classflag == 1:
                                allprediction = self.predict_logits_from_preprocessed_data(nowdata)
                                prediction, classprediction = allprediction[0].cpu(), allprediction[1].cpu()
                                _, classprediction = torch.max(classprediction, dim=-1, keepdim=True)
                            else:
                                prediction = self.predict_logits_from_preprocessed_data(nowdata).cpu()
                            ret.append(prediction)
                            my_data = prediction.squeeze(0).tolist()
                            my_data = [element for sublist in my_data for element in sublist]
                            my_data = nowframe + my_data
                            writer.writerow(my_data)
                            if classflag == 1:
                                classprediction = classprediction.squeeze(0).squeeze(-1).tolist()
                                classprediction = nowframe + classprediction
                                classpredata.append(classprediction)
                    if classflag == 1:
                        classcsv = savecsv.replace("val_pre","val_class_pre")
                        with open(classcsv, 'w', newline='') as csvfile:
                            writer = csv.writer(csvfile)
                            for classpre in classpredata:
                                writer.writerow(classpre)
                else:
        ...
        with torch.no_grad():
            ...
            for params in self.list_of_parameters:
                ...
                if not isinstance(self.network, OptimizedModule):
                    ...
                    if self.timeall == []:
                        print(f"Total parameters: {total_params_millions:.3f}M")
                else:
                ...
                if lzz_flage == 1:
                    if prediction is None:
                        if classflag == 1:
                            allprediction = self.predict_sliding_window_return_logits(data)
                            prediction = allprediction[0]
                            classprediction = allprediction[1]
                            classprediction = [torch.sigmoid(i) for i in classprediction]
                        else:
                            prediction = self.predict_sliding_window_return_logits(data)
                        # 计算均值，指定 dim=0 表示在第0维度上求均值，结果维度为 [1, 96, 1]
                        ...
                        if classflag == 1:
                            classprediction = torch.stack(classprediction)
                            classprediction = torch.mean(classprediction, dim=0)
                            classprediction = classprediction.to('cpu')
                            threshold = 0.5
                            classprediction = (classprediction > threshold).int()
                else:
                    if prediction is None:
        ...
        with torch.no_grad():
            with torch.autocast(self.device.type, enabled=True) if self.device.type == 'cuda' else dummy_context():
                ...
                if lzz_flage == 1:
                    do_on_device = True
                    results_device = self.device if do_on_device else torch.device('cpu')
                    input_image = input_image.unsqueeze(0).to(results_device)
                    start_time = time.time()
                    predicted_logits = self.network(input_image)
                    end_time = time.time()
                    elapsed_time = end_time - start_time
                    self.timeall.append(elapsed_time)
...
def predict_entry_point():
    ...
    parser = argparse.ArgumentParser(description='Use this to run inference with nnU-Net. This function is used when '
                                                 ...
                                                 'model. This is useful when the nnunet environment variables '
                                                 '(nnUNet_results) are not set.')
    parser.add_argument('-i', type=str, required=True, default='/root/lzz2/cardiac_cycle/mamba/try_umamba/U-Mamba/data/nnUNet_preprocessed/Dataset701_AbdomenCT/nnUNetPlans_3d_fullres',
                        help='input folder. Remember to use the correct channel numberings for your files (_0000 etc). '
                             'File endings must be the same as the training dataset!')
    parser.add_argument('-o', type=str, required=True, default='/root/lzz2/cardiac_cycle/mamba/try_umamba/U-Mamba/data/nnUNet_results/Dataset701_AbdomenCT/nnUNetTrainerUMambaEnc__nnUNetPlans__3d_fullres/predict',
                        help='Output folder. If it does not exist it will be created. Predicted segmentations will '
                             'have the same name as their source images.')
    parser.add_argument('-d', type=str, required=True, default='nnUNetPlans',
...
if __name__ == '__main__':
    ...
    predictor.initialize_from_trained_model_folder(
        join(nnUNet_results, 'Dataset701_AbdomenCT/nnUNetTrainerUMambaEnc__nnUNetPlans__3d_fullres'),
        use_folds="all",
        checkpoint_name='checkpoint_best.pth',
    )
    predictor.predict_from_files(join(nnUNet_preprocessed, 'Dataset701_AbdomenCT/Apical_4C'),
                                 join(nnUNet_results, 'Dataset701_AbdomenCT/nnUNetTrainerUMambaEnc__nnUNetPlans__3d_fullres/val_pre_levelmeanv3'),
                                 save_probabilities=False, overwrite=True,
                                 num_processes_preprocessing=1, num_processes_segmentation_export=1,
                                 folder_with_segs_from_prev_stage=None, num_parts=1, part_id=0)
    alltime = sum(predictor.timeall)
    lens = len(predictor.timeall)
    print("总用时：  ", alltime )
    print("lens:  ", lens )
    print(f"Execution time: {alltime / lens:.6f} seconds")
    # predictor.predict_from_files(join(nnUNet_raw, 'Dataset701_AbdomenCT/nnUNetPlans_3d_fullres'),
    #                              join(nnUNet_raw, 'Dataset701_AbdomenCT/nnUNetPlans_3d_fullres_predfullres'),
    #                              save_probabilities=False, overwrite=True,
