...
def get_identifiers_from_splitted_dataset_folder(folder: str, file_ending: str):
    ...
    if lzz_flag == 1:
        folder = os.path.join(folder.rsplit("/", 1)[0], "Apical_4C")
        # folder = os.path.join(folder.rsplit("/", 1)[0], "Parasternal_4C")
        # folder = os.path.join(folder.rsplit("/", 1)[0], "Basal_4C")
        valdatalistpath = os.path.join(folder, "valdatalist.csv")
        # valdatalistpath = os.path.join(folder, "valdatalist1.csv")
        # valdatalistpath = os.path.join(folder, "valdatalist2.csv")
        # valdatalistpath = os.path.join(folder, "valdatalist3.csv")
        # valdatalistpath = os.path.join(folder, "traindatalist.csv")
        files = read_csv(valdatalistpath)
...
def create_lists_from_splitted_dataset_folder(folder: str, file_ending: str, identifiers: List[str] = None) -> List[
    ...
    if lzz_flag == 1:
        ...
        for f in identifiers:
            # list_of_lists.append([join(folder, f + ".npy")])
            flist = [os.path.join(videopath, f + ".avi"),
                     os.path.join(videopath, f + ".AVI"),
                     os.path.join(videopath, f + ".mp4")]
            for video_path in flist:
                if os.path.exists(video_path):
                    list_of_lists.append([video_path])
