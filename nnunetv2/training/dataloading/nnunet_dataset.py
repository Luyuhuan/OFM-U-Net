...
class nnUNetDataset(object):
    def __init__(self, folder: str, case_identifiers: List[str] = None,
        ...
        for c in case_identifiers:
            ...
            if lzz_flag == 1:
                # self.dataset[c]['properties_file'] = join(folder, "all.pkl")
                self.dataset[c]['properties_file'] = r"/root/lzz2/cardiac_cycle/mamba/try_umamba/U-Mamba/data/nnUNet_preprocessed/Dataset701_AbdomenCT/Apical_4C/all.pkl"
        ...
        if lzz_flag == 1:
            self.videopath = r"/root/lzz2/cardiac_cycle/data/all_data_apex_direction/Apical"
            self.jsonpath = r"/root/lzz2/cardiac_cycle/data/all_data_regressionlabel_apex_direction/Apical"
            # self.videopath = r"/root/lzz2/cardiac_cycle/data/all_data_apex_direction/Parasternal"
            # self.jsonpath = r"/root/lzz2/cardiac_cycle/data/all_data_regressionlabel_apex_direction/Parasternal"
            # self.videopath = r"/root/lzz2/cardiac_cycle/data/all_data_apex_direction/Basal"
            # self.jsonpath = r"/root/lzz2/cardiac_cycle/data/all_data_regressionlabel_apex_direction/Basal"
            self.dataanno = {}
    ...
    def load_case(self, key):
        ...
        entry = self[key]
        if lzz_flag == 1:
            if key not in self.dataanno:
                self.dataanno[key] = {}
                datalist = [os.path.join(self.videopath, key + ".avi"),
                            os.path.join(self.videopath, key + ".AVI"),
                            os.path.join(self.videopath, key + ".mp4")]
                for video_path in datalist:
                    if os.path.exists(video_path):
                        data = video_path
                seg = os.path.join(self.jsonpath, key + ".json")
                self.dataanno[key]["data"] = data
                self.dataanno[key]["seg"] = seg
            return self.dataanno[key]["data"], self.dataanno[key]["seg"], entry['properties']
